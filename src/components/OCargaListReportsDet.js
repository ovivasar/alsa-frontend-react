...
export default function OCargaListReportesDet() {
  ...
  const cargaRegistro = async () => {
    ...
    var partes;
    var codigo;
    partes = params.codigo.split("-"); // Divide la cadena en partes usando "-" como separador
    codigo = partes[0]; // Obtiene la primera parte que es el código
    if (params.valor_vista==="cliente"){
        response = await fetch(`${back_host}/ocargaplancrosstab2det/${strFechaIni}/${strFecha}/${codigo}`);
    }else{
      if (params.valor_vista==="producto"){
        response = await fetch(`${back_host}/ocargaplancrosstab3/${strFechaIni}/${strFecha}`);
      }else{
        if (params.valor_vista==="vendedor"){
          response = await fetch(`${back_host}/ocargaplancrosstab4/${strFechaIni}/${strFecha}`);
        }
        else{
          if (params.valor_vista==="zona"){
            response = await fetch(`${back_host}/ocargaplancrosstab5/${strFechaIni}/${strFecha}`);
          }
          else{
            //El pedidos nomas
            response = await fetch(`${back_host}/ocargaplancrosstab1det/${strFechaIni}/${strFecha}/${codigo}`);
          }
        }
      }
    }
  ...
  const columnas = [
    { //name:'DESCRIPCION', 
      name:params.codigo, //lleva descripcion completa
